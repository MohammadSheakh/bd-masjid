...
description: Generate or update architectural, invariant-focused README.md files with visual Mermaid diagrams for modules in backend-nest-prisma/src/features/. Use when documenting a backend feature, clarifying module boundaries, defining database ownership, visualizing component architecture, or creating feature-level READMEs.
---

# Backend Feature README Generator

Creates and maintains high-value, architectural, super accurate, brutal honest  `README.md` documents for bounded backend modules in `backend-nest-prisma/src/features/`.
...
Every `README.md` in `backend-nest-prisma/src/features/<feature>/` should follow this standardized schema:
...
| :--- | :--- | :--- |
| `FeatureController` | HTTP Controller | `./controllers/feature.controller.ts` |
| `FeatureService` | Domain Orchestration | `./services/feature.service.ts` |
| `InjectedModuleDependency` | Cross-Module Dependency | `../other-module/services/other.service.ts` |
| `PrismaService` | Database ORM | `@app/database` (`backend-nest-prisma/libs/database`) |
...
1. Check Prisma schema (`backend-nest-prisma/prisma/schema.prisma` or module `.prisma` file).
